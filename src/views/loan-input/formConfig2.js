...
function phone_validator(rule, value, callback) {
    let reg = /^1[3|4|5|7|8][0-9]\d{8}$/;
    if (!value) return callback(new Error('电话不能为空'));
    if (value.trim() === '') return callback(new Error('电话不能为空'));
    if (!reg.test(value)) return callback(new Error('请输入正确的电话号码'));

    // 验证电话与移动电话的值一致
    console.log(this.form)
    if (this.form.phone !== this.form.mobile_phone) return callback(new Error('电话与移动电话不一致'));
    callback()
}

// 验证前缀符号
function checkPrefixWrapper(prefix) {
    console.log(prefix)
    return function (rule, value, callback) {
        console.log(value)
        console.log(this)
        if (!value.startsWith(prefix)) return callback(new Error('必须以' + prefix + '开头'))
        callback()
    }
}
